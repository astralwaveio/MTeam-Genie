...
class MTeamManager:
    ...
    def get_torrent_download_url(self, torrent_id: str) -> Optional[str]:
        if not self.config.MT_APIKEY or not self.config.MT_HOST: return None
        ...
        try:
            ...
            for t in torrents_list_raw:
                ...
                display_text = (f"<b>{html.escape(title_to_display)}</b>\n"
                                f"  💾 {QBittorrentManager._format_bytes(int(t.get('size', 0)))} | 🆔 {t.get('id', 'N/A')} {format_mteam_discount(t.get('status', {}).get('discount'))}\n"
                                f"  📂 {html.escape(get_mteam_category_name(str(t.get('category', '0'))))}"
                                + (
                                    f"\n  📝 <i>{html.escape(subtitle_text[:70] + ('...' if len(subtitle_text) > 70 else subtitle_text))}</i>" if subtitle_text else "")).strip()
                formatted_torrents.append(
                    {"id": t.get("id"), "name": title_to_display, "display_text": display_text, "api_details": t})
...
class QBittorrentManager:
    ...
    @staticmethod
    def _format_bytes(b: Union[int, str]) -> str:
        try:
            b_int = int(b)
    ...
    async def get_all_torrents_info(self, page: int = 1, items_per_page: int = 10) -> Tuple[
        ...
        try:
            torrents_list = self.client.torrents_info(status_filter='all') or []
            sorted_torrents = sorted(torrents_list, key=lambda t: t.name.lower())
            ...
            torrents_for_page = sorted_torrents[(current_page - 1) * items_per_page: current_page * items_per_page]
            parts = [
                f"{self._get_torrent_state_emoji(t.state, t.progress)} <b>{html.escape(t.name[:60])}{'...' if len(t.name) > 60 else ''}</b>\n  {t.progress * 100:.1f}% | {self._format_bytes(t.size)} | [{html.escape(t.category) if t.category else '未分类'}]"
    ...
    async def get_all_categories(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            msg = "📚 <b>qB分类列表:</b>\n" + (
                "\n".join(
                    f"  👉  <code>{html.escape(name)}</code>" for name in categories) if categories else "  (无分类)")
            return True, msg
        except Exception as e:
    ...
    async def set_torrent_category_by_hash(self, torrent_hash: str, new_category: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            name_esc = html.escape(current_torrent.name[:50]) + ('...' if len(current_torrent.name) > 50 else '')
...
async def _display_torrent_page(update_obj: Union[Update, telegram.CallbackQuery], context, config, qb_manager,
    ...
    # Determine chat_id and perform auth check early
...
async def handle_add_category_selection(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        f"🔄 添加中 (ID: <code>{mt_id}</code>, 分类: {html.escape(selected_category) or '无'})...", reply_markup=None,
        parse_mode=ParseMode.HTML)
    qb_manager: QBittorrentManager = context.bot_data['qb_manager']
    ...
    try:
        ...
        await context.bot.send_message(query.message.chat_id, message, parse_mode=ParseMode.HTML)
...
async def received_setcat_mt_id(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if qb_manager.connect_qbit():
        try:
            ...
            if info: name, current_cat = info[0].name, info[0].category or "<i>(无分类)</i>"
        finally:
            qb_manager._disconnect()

    _, categories = await qb_manager.get_qb_category_names_list()
    buttons = [[InlineKeyboardButton(f"📁 {cat}", callback_data=f"{MOD_CAT_PREFIX}{cat}")] for cat in categories[:15] if
    ...
    await processing_msg.edit_text(
        f"任务: {html.escape(name)}\nID: {html.escape(mt_id)}, HASH: <code>{torrent_hash[:8]}..</code>\n"
        f"当前分类: {current_cat}\n<b>请选择新分类:</b>",
...
async def received_del_option(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(f"🔄 删除中 (HASH: <code>{torrent_hash[:8]}..</code>)...", reply_markup=None,
                                  parse_mode=ParseMode.HTML)
...
async def display_search_results_page(update: Union[Update, telegram.CallbackQuery], context: ContextTypes.DEFAULT_TYPE,
    ...
    kbd_rows = [[InlineKeyboardButton(f"📥 选择 (ID: {t['id']})", callback_data=f"{SEARCH_SELECT_PREFIX}{t['id']}")] for
                t in torrents]

    pg_btns = []
    if page_num > 0: pg_btns.append(
        InlineKeyboardButton("⬅️ 上页", callback_data=f"{SEARCH_PAGE_PREFIX}{page_num - 1}"))
    if (page_num + 1) < total_pages_api: pg_btns.append(
        InlineKeyboardButton("➡️ 下页", callback_data=f"{SEARCH_PAGE_PREFIX}{page_num + 1}"))
    ...
    full_text = header + ("\n\n" + "\n\n".join(content_parts) if content_parts else "") + \
                (f"\n\n📄 第 {curr_page_api}/{total_pages_api} 页" if total_pages_api > 0 else "")
...
async def handle_search_result_selection(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not mt_id.isdigit():
        await query.message.edit_text("选择错误，ID无效。", reply_markup=None)
        return SHOWING_SEARCH_RESULTS
    context.user_data['add_mt_id'] = mt_id
    await query.message.edit_text(f"已选种子ID: <code>{mt_id}</code>. 请选择qB分类:", reply_markup=None,
                                  parse_mode=ParseMode.HTML)

    config: Config = context.bot_data['config']
    qb_manager: QBittorrentManager = context.bot_data['qb_manager']
    _, categories = await qb_manager.get_qb_category_names_list()
    buttons = [[InlineKeyboardButton(f"📁 {cat}", callback_data=f"{ADD_CAT_PREFIX}{cat}")] for cat in categories[:15] if
               isinstance(categories, list)]
    buttons.extend([
    ...
    await context.bot.send_message(query.message.chat_id, "请为下载选择qB分类:",
                                   reply_markup=InlineKeyboardMarkup(buttons))
    return SELECTING_ADD_CATEGORY
...
async def handle_search_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.answer()
    try:
        await query.message.edit_text("搜索已取消。", reply_markup=None)
    except Exception:
        pass
    return await cancel_conversation(update, context)
...
def main_bot() -> None:
    ...
    app.add_handler(CommandHandler("help", help_command))  # help 也可作为独立命令
    ...
    conv_handler = ConversationHandler(
        ...
        name="mteam_qb_conv", allow_reentry=True
    )
